...
Add to your shell rc file:
  zsh:  eval "$(herdctl shell zsh)"   → ~/.zshrc
  bash: eval "$(herdctl shell bash)"  → ~/.bashrc
...
# Add to ~/.zshrc: eval "$(herdctl shell zsh)"
...
alias qps='herdctl quick'

# Agent management
alias ant='herdctl add'
alias bp='herdctl send'
alias int='herdctl interrupt'

# Session navigation
alias rnt='herdctl attach'
alias lnt='herdctl list'
alias snt='herdctl status'
alias vnt='herdctl view'
alias znt='ntm zoom'

# Output management
alias cpnt='herdctl copy'
alias svnt='herdctl save'

# Utilities
alias ncp='herdctl palette'
alias knt='herdctl kill'
alias dnt='herdctl deps'
...
_ntm_complete_sessions() {
  local sessions
  sessions=(${(f)"$(herdctl list 2>/dev/null | awk -F: '{gsub(/^[[:space:]]+/, "", $1); print $1}')"})
  _describe 'session' sessions
}

_ntm_complete_ensemble_presets() {
  local presets
  presets=(${(f)"$(herdctl ensemble presets --format=json 2>/dev/null | sed -n 's/.*\"name\"[[:space:]]*:[[:space:]]*\"\\([^\"\\\\]\\+\\)\".*/\\1/p' | sort -u)"})
  _describe 'preset' presets
}

_ntm_complete_mode_ids() {
  local modes
  modes=(${(f)"$(herdctl modes list --format=json --all 2>/dev/null | sed -n 's/.*\"id\"[[:space:]]*:[[:space:]]*\"\\([^\"\\\\]\\+\\)\".*/\\1/p' | sort -u)"})
...
if [[ -n "$TMUX" ]]; then
  ...
  bindkey -r '^[[17~'
  _ntm_tmux_popup() {
    tmux popup -E -w 80% -h 80% "herdctl palette"
  }
...
# Add to ~/.bashrc: eval "$(herdctl shell bash)"
...
`)
	...
	b.WriteString(`# Tab completions
	_ntm_list_sessions() {
	  herdctl list 2>/dev/null | awk -F: '{gsub(/^[[:space:]]+/, "", $1); print $1}'
	}

	_ntm_list_ensemble_presets() {
	  herdctl ensemble presets --format=json 2>/dev/null | sed -n 's/.*"name"[[:space:]]*:[[:space:]]*"\([^"]\+\)".*/\1/p' | sort -u
	}
...
`)
	...
	b.WriteString(`# Short aliases for ntm commands
# Session creation
abbr -a cnt 'herdctl create'
abbr -a sat 'herdctl spawn'
abbr -a qps 'herdctl quick'

# Agent management
abbr -a ant 'herdctl add'
abbr -a bp 'herdctl send'
abbr -a int 'herdctl interrupt'

# Session navigation
abbr -a rnt 'herdctl attach'
abbr -a lnt 'herdctl list'
abbr -a snt 'herdctl status'
abbr -a vnt 'herdctl view'
abbr -a znt 'ntm zoom'

# Output management
abbr -a cpnt 'herdctl copy'
abbr -a svnt 'herdctl save'

# Utilities
abbr -a ncp 'herdctl palette'
abbr -a knt 'herdctl kill'
abbr -a dnt 'herdctl deps'
...
`)
	...
	function __fish_ntm_sessions
	  herdctl list 2>/dev/null | string match -r '^\s*\S+' | string trim
	end

	function __fish_ntm_ensemble_presets
	  herdctl ensemble presets --format=json 2>/dev/null | string match -rg '"name"\s*:\s*"([^"]+)"' '$1' | sort -u
	end
	...
	bind \e\[17~ 'commandline -r "herdctl palette"; commandline -f execute'
	`)
