...
`,
		Example: `  herdctl ensemble project-diagnosis "What are the main issues?"
  herdctl ensemble idea-forge "What features should we add next?"
  herdctl ensemble spawn mysession --preset project-diagnosis --question "..."`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
			if len(args) < 2 {
				return fmt.Errorf("ensemble name and question required (usage: herdctl ensemble <ensemble-name> <question>)")
			}
...
Flags:
  ...
  --format       Output format: text, json, yaml`,
		Example: `  herdctl ensemble stop
  herdctl ensemble stop my-ensemble-session
  herdctl ensemble stop --force
  herdctl ensemble stop --no-collect --quiet`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			session := ""
			if len(args) > 0 {
...
func printSynthesisResumeHint(session, runID, format string) {
	...
	fmt.Fprintf(os.Stderr, "Resume with: herdctl ensemble synthesize %s --stream --resume --run-id %s%s\n", session, runID, formatFlag)
}
...
func formatBeadDescription(ctx *exportFindingsContext, f exportFinding) string {
	...
	b.WriteString(fmt.Sprintf("---\n*Generated by herdctl ensemble export-findings at %s*\n", time.Now().UTC().Format(time.RFC3339)))
...
Formats:
  ...
  --format=json           - Machine-readable JSON
  --format=yaml           - YAML format`,
		Example: `  herdctl ensemble provenance abc123def456
  herdctl ensemble provenance --all
  herdctl ensemble provenance --stats
  herdctl ensemble provenance --all --format=json`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			session := opts.Session
			res, err := resolveEnsembleStateCommandSession(session, cmd.OutOrStdout())
...
Use 'herdctl ensemble list-checkpoints' to see available runs.`,
		Example: `  herdctl ensemble resume my-ensemble-run
  herdctl ensemble resume my-ensemble-run --skip-done
  herdctl ensemble resume my-ensemble-run --format json`,
...
The mode's checkpoint will be updated with the new output.`,
		Example: `  herdctl ensemble rerun-mode my-run deductive
  herdctl ensemble rerun-mode my-run A1 --format json`,
...
Use --all to remove all checkpoints regardless of age.`,
		Example: `  herdctl ensemble clean-checkpoints
  herdctl ensemble clean-checkpoints --max-age 24h
  herdctl ensemble clean-checkpoints --all
  herdctl ensemble clean-checkpoints --dry-run`,
		RunE: func(cmd *cobra.Command, args []string) error {
